...
| urlPrefixAllowed(urls)                                                                                                                                                                                                                                                                                                       | 设置当前PyBurp标签允许处理的url前缀，通过`urls.add(url)`添加，需要配合上面4个方法使用                                                                                         |
...
| [bambdas.py](./src/main/resources/examples/bambdas.py)                                       | 快速从Proxy历史记录中查找感兴趣的内容                                                                                          |
...
| [customise_context_menu.py](./src/main/resources/examples/customise_context_menu.py)         | 注册上下文菜单示例，包含NoSQL注入，条件竞争，权限绕过等菜单项                                                                              |
...
| [highlight_interesting_http.py](./src/main/resources/examples/highlight_interesting_http.py) | 高亮显示感兴趣的请求，添加Notes示例，通常情况下更建议使用被动扫描                                                                            |
...
| MESSAGE_EDITOR   | 接受一个[MessageEditorHttpRequestResponse](https://portswigger.github.io/burp-extensions-montoya-api/javadoc/burp/api/montoya/ui/contextmenu/MessageEditorHttpRequestResponse.html)类型的参数，在需要修改http消息面板内容时使用，通常情况下使用`SELECTED_TEXT`类型更方便 |
